...
function Expand-Patch {
    param([string]$Payload, [string]$ExpectedHash, [string]$OutputBase)
    if (-not (Test-Path $Payload)) { throw "Missing patch payload: $Payload" }
    $actual = (Get-FileHash $Payload -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($actual -ne $ExpectedHash) { throw "Patch checksum mismatch for $Payload : $actual" }
    $b64 = (Get-Content $Payload -Raw -Encoding ASCII).Trim()
    $gzPath = "$OutputBase.gz"
    [IO.File]::WriteAllBytes($gzPath, [Convert]::FromBase64String($b64))
    $input = [IO.File]::OpenRead($gzPath)
    $output = [IO.File]::Create($OutputBase)
    try {
...
Write-Host '=== Reconstruct verified source ==='
$chunkInfo = @(
...
if ((Get-FileHash source.b64 -Algorithm SHA256).Hash.ToLowerInvariant() -ne '7e406b7bddabd6aa206cf151e3313af67e2d33baf1bc9cc25509c71f45cb5039') { throw 'Source base64 checksum mismatch' }
[IO.File]::WriteAllBytes('source.tar.xz', [Convert]::FromBase64String($b64))
if ((Get-FileHash source.tar.xz -Algorithm SHA256).Hash.ToLowerInvariant() -ne '796cf89de00eb700817a0aa16e38b5c31955218ca857fd7b27f84bd1a4fae12d') { throw 'Source archive checksum mismatch' }
tar -xf source.tar.xz
if ($LASTEXITCODE -ne 0) { throw 'Source extraction failed' }
if (-not (Test-Path 'src/KukaManager/KukaManager.csproj')) { throw 'Project reconstruction failed' }
...
Write-Host '=== Apply 1.1.0 ==='
Expand-Patch 'kuka-ui/patch.gz.b64' 'a14ad60f0d3ae54af10cca88dcfa7a568e9927a5263c67be912f420d38abf07e' 'kuka-ui-110.patch'
...
$project = [Regex]::Replace($project, '<Version>[^<]+</Version>', '<Version>1.1.0</Version>', 1)
[IO.File]::WriteAllText($projectPath, $project, [Text.UTF8Encoding]::new($false))

Write-Host '=== Apply 1.1.1 ==='
Expand-Patch 'kuka-ui/fix-111.patch.gz.b64' '2c7036849ba76d453a4976ec9a2adc23ff4b461bc56a89119b066663b6b03665' 'kuka-ui-111.patch'
...
$project = Get-Content $projectPath -Raw -Encoding UTF8
$project = [Regex]::Replace($project, '<Version>[^<]+</Version>', '<Version>1.1.1</Version>', 1)
[IO.File]::WriteAllText($projectPath, $project, [Text.UTF8Encoding]::new($false))

Write-Host '=== Apply 1.2.0 UI foundation ==='
Expand-Patch 'kuka-ui/ui-120.patch.gz.b64' '53becf01bbb09769ae0d6c9a4c362f16b9f3818754995dc03f63f713e95d955b' 'kuka-ui-120.patch'
...
if ($project -notmatch '<ApplicationManifest>') {
    $project = $project.Replace('<ApplicationIcon>kuka.ico</ApplicationIcon>', "<ApplicationIcon>kuka.ico</ApplicationIcon>`r`n    <ApplicationManifest>app.manifest</ApplicationManifest>")
}
[IO.File]::WriteAllText($projectPath, $project, [Text.UTF8Encoding]::new($false))
...
if ((Get-Content 'src/KukaManager/app.manifest' -Raw -Encoding UTF8) -notmatch 'PerMonitorV2') { throw 'PerMonitorV2 manifest missing' }
if ((Get-Content 'src/KukaManager/Utils/AppPaths.cs' -Raw -Encoding UTF8) -notmatch '1\.2\.0') { throw 'AppPaths version is not 1.2.0' }
if ((Get-Content $projectPath -Raw -Encoding UTF8) -notmatch '<Version>1\.2\.0</Version>') { throw 'Project version is not 1.2.0' }

Write-Host '=== Restore and build ==='
Invoke-Dotnet restore '.\src\KukaManager\KukaManager.csproj' '--source' 'https://api.nuget.org/v3/index.json'
Invoke-Dotnet build '.\src\KukaManager\KukaManager.csproj' '-c' 'Release' '--no-restore'
Invoke-Dotnet restore '.\tests\KukaManager.SmokeTests\KukaManager.SmokeTests.csproj' '--source' 'https://api.nuget.org/v3/index.json'
Invoke-Dotnet build '.\tests\KukaManager.SmokeTests\KukaManager.SmokeTests.csproj' '-c' 'Release' '--no-restore'

Write-Host '=== Run smoke tests ==='
Invoke-Dotnet run '--project' '.\tests\KukaManager.SmokeTests\KukaManager.SmokeTests.csproj' '-c' 'Release' '--no-build'

Write-Host '=== Publish self-contained win-x64 ==='
Invoke-Dotnet restore '.\src\KukaManager\KukaManager.csproj' '-r' 'win-x64' '--source' 'https://api.nuget.org/v3/index.json'
Invoke-Dotnet publish '.\src\KukaManager\KukaManager.csproj' '-c' 'Release' '-r' 'win-x64' '--self-contained' 'true' '--no-restore' '-o' '.\dist\KukaManager'
...
if (-not (Test-Path '.\KukaManager-CSharp-1.2.0-win-x64.zip')) { throw 'Package missing' }
Write-Host 'KukaManager 1.2.0 package ready.'
