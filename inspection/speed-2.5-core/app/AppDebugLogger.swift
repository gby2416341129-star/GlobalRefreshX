...
enum AppDebugLogger {
    ...
    private static let logger = Logger(subsystem: "com.isil.speed", category: "diagnostics")
    private static let logQueueKey = DispatchSpecificKey<Void>()
    private static let logQueue: DispatchQueue = {
        let queue = DispatchQueue(label: "com.isil.speed.appDebugLog", qos: .utility)
    ...
    private static var debugModeEnabledCache = UserDefaults.standard.bool(forKey: debugModeKey)
    ...
    static var isDebugModeEnabled: Bool {
        ...
        get {
            debugModeLock.lock()
            let value = debugModeEnabledCache
            debugModeLock.unlock()
            return value
        }
        set {
            debugModeLock.lock()
            debugModeEnabledCache = newValue
            debugModeLock.unlock()
    ...
    static func registerBackgroundFlush() {
        // Release-normal path: diagnostics are opt-in. Avoid lifecycle observers when
        // debug logging is disabled so normal 120 Hz operation has zero logging wakeups.
    ...
    static func log(_ message: @autoclosure () -> String, file: StaticString = #fileID, line: UInt = #line) {
        // Keep the release-normal path genuinely cheap: callers can freely include
        // interpolation without paying to build the String while diagnostics are off.
        guard isDebugModeEnabled else { return }
        append(message(), persistImmediately: false, file: file, line: line)
    }

    // 生命周期、内存警告和保活切换使用此入口，避免系统直接终止时最后状态只留在内存。
    static func logCritical(_ message: @autoclosure () -> String, file: StaticString = #fileID, line: UInt = #line) {
        guard isDebugModeEnabled else { return }
...
enum DebugDiagnosticsMonitor {
    ...
    static func startIfNeeded() {
        // A stale per-monitor preference must never resurrect diagnostic timers/display links
        // after the user has disabled the master debug switch.
...
// Release builds deliberately carry only no-op diagnostic facades. This keeps
// lifecycle call sites simple while excluding watchdogs, MetricKit snapshots,
// log queues, frame-stutter display links, and process-exit observers.
enum AppDebugLogger {
    static var isDebugModeEnabled: Bool {
        get { false }
        set { _ = newValue }
...
enum DiagnosticsRuntimeState {
    ...
    static func reset() {}
    static func refreshAppState() {}
    static func updateAppState(_ state: @autoclosure () -> String) {}
    static func updateCurrentPage(_ page: @autoclosure () -> String) {}
    static func updatePiPState(_ state: @autoclosure () -> String) {}
    static func updateDisplaySleepState(_ state: @autoclosure () -> String) {}
    static func updatePiPSurfaceState(_ state: @autoclosure () -> String) {}
    static func recordUserAction(_ action: @autoclosure () -> String) {}
    static func updateLastEvent(_ event: @autoclosure () -> String) {}
    static func snapshotText(includeAudio: Bool = true) -> String { "" }
    static var isForegroundActive: Bool { UIApplication.shared.applicationState == .active }
}

enum PerformanceDiagnosticsLogger {
    static var isEnabled: Bool { false }
    static func setEnabled(_ isEnabled: Bool) { _ = isEnabled }
    static func startIfNeeded() {}
    static func stop() {}
    static func setRuntimeSamplingSuppressed(_ isSuppressed: Bool, reason: String) {
        _ = isSuppressed; _ = reason
    }
    static func currentSnapshotText() -> String { "" }
}
...
enum ProcessTerminationDiagnostics {
    ...
    static func recordCheckpoint(
        ...
        _ = pipSnapshot; _ = persistImmediately
    }
    static func recordMemoryWarning() {}
    static func markGracefulTermination(reason: String) { _ = reason }
    static func exportText() -> String { "" }
    static func reset() {}
}
